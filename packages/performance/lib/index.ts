type TimingEntry = { name: string; loadTime: number };
type TimingTrace = {
  page: TimingEntry;
...
type TraceFunc = (timing: TimingTrace) => void;
...
export default class Performance {
  ...
  onTrace = (trace: TraceFunc) => {
    ...
    window.onload = e => {
      ...
      const data: TimingTrace = { page, resource, fp, fcp };
  ...
  /** 资源加载是否超时 */
  #isTimeout = (stamp: number): boolean => stamp > this.#timeout;
  ...
  getPageLoadTime = (): TimingEntry => {
    const [timing] = this.#performance.getEntriesByType('navigation');
    const { name, domComplete } = timing as PerformanceNavigationTiming;
  ...
  getResourceTime = (): TimingEntry[] => {
    ...
    resources.forEach(({ name, startTime, responseEnd }) => {
      ...
      this.#isTimeout(loadTime) && buffer.push({ name, loadTime });
    });
   ...
   */
  getFPTime = (): number => {
    const [timing] = this.#performance.getEntriesByType(
      'paint',
    ) as PerformancePaintTiming[];
    return timing?.startTime || 0;
  };
   ...
   */
  getFCPTime = (): number => {
    const timings = this.#performance.getEntriesByType(
      'paint',
    ) as PerformancePaintTiming[];
