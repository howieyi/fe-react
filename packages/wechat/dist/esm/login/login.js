 ...
 * @returns
 */
export const webpageLogin = (appId, isSilent = false, redirectUrl = window.location.href) => {
    if (!isWeChatBrowser())
        return;
    if (!appId)
    ...
    window.location.replace(`https://open.weixin.qq.com/connect/oauth2/authorize?appid=${appId}&redirect_uri=${encodeURIComponent(redirectUrl)}&response_type=code&scope=${scope}&state=ST-${Date.now()}#wechat_redirect`);
    return '';
};
